...
Parrotfish density estimates are for parrotfishes that were at least 15cm fork length. We classified major corallivores as *Sc. guacamaia, Sc. taeniopterus, Sc. vetula, Sp. viride*, see manuscript for details. 
...
transect_level_model_output %>% filter(p_value<0.05)
```

**Fig. S2.** Abundance of *Orbicella annularis*, Branching *Porites* spp., *Porites astreoides*, and *Siderastrea siderea* coral colonies with recent parrotfish predation scars per 30 m2 reef area in response to the diversity of target coral taxa within the area. We calculated diversity using the Shannon Diversity Index (H’) based on the richness and evenness of the number of target coral colonies within a given transect. We focused on evaluated the abundance of corals grazed these four taxa as they accounted for >80% of observed colonies with recent predation scars. 

```{r Fig S2: Abundance corals grazed by coral diversity, fig.height = 3.15, fig.width = 8.46}
...
mean_scar_size_col_level_model_output %>% filter(p_value<0.05)
```
...
pct_grazed_col_level_model_output %>% filter(p_value<0.05)
```
