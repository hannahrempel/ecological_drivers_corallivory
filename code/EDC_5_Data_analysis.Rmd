...
coral_scar_df %>% filter(n_new_scars>0)
```
...
Notes: Parrotfish density estimates are for parrotfishes that were at least 15cm fork length. We classified major corallivores as *Sc. guacamaia, Sc. taeniopterus, Sc. vetula, Sp. viride*. 

```{r Summary of parrotfish density data}
# Transect-level summary of fish density
major_corallivores_df <- fish_df %>% 
  filter(species %in% c("Scarus guacamaia", "Scarus taeniopterus", "Scarus vetula", "Sparisoma viride")) %>%
...
pf_density_transect <- fish_df %>% 
  ...
  mutate(n_major_corallivores = replace_na(n_major_corallivores, 0), #replacing NA for surveys with no parrotfish
         density_major_corallivores = n_major_corallivores/survey_area_m2*100) %>% #(fish/100 m2) 
  ungroup()
...
mean_scar_size_by_region <- coral_scar_df %>% 
  filter(total_grazed_cm2>0) %>%
  group_by(region) %>%
  dplyr::reframe(mean_scar_size_cm2=mean(scar_med_cm2),
...
#Variation in corallivory among regions

##Regional variation in parrotfish density, coral cover, and corallivory intensity
...
```{r Figure 2: parrotfish + coral + corallivory patterns across sites and regions, echo=FALSE, message=FALSE}
...
fig2a_parrotfish_density <- ggplot()+
  ...
  scale_x_continuous(limits = c(0, 17), expand = c(0.025, 0)) + #leaves enough space for points to not cross 0
...
fig2d_percent_scar_area <- ggplot() +
  ...
  scale_color_manual(values = coral_palette) + 
  facet_grid(rows = vars(region), scales = "free_y", switch = "y", space = "free") +
  labs(title = "d", x = "Coral area grazed (%)") +
  theme_bw()+
  theme(axis.title.x = element_text(margin = margin(t = 2.2, r = 0, b = 0.8, l = 0, unit = "mm")),
        plot.title = element_text(size = 12, face = "bold"),
        plot.margin = margin(0.2, 0.2, 0.2, 0.2, unit = "cm"),
...
fig2a_b <- ggpubr::ggarrange(fig2a_parrotfish_density,  
                                                      fig2b_coral_cover,
                                                      ncol=2,
...
#bottom panel (c-d)
fig2c_d <- ggpubr::ggarrange(fig2c_scar_density, 
                                            fig2d_percent_scar_area,
                                            ncol=2,
...
fig2c_d_with_graphics <- cowplot::ggdraw() +
  draw_plot(corallivory_paried_fig)+ #adds the plot (order matters to overlay the layers correctly)
...
We used Kruskal-Wallis tests to evaluate the effect of region on (1) density of major corallivores and (2) percent coral cover. We used KW tests because the relationship between parrotfish density and region had a non-normal distribution of the residuals and coral cover had heterogenous variance by region. As tests were significant, we conducted post-hoc Dunn’s tests of pairwise comparisons among regions using the Benjamini-Hochberg correction for multiple comparisons (this method decreases the false discovery rate compared to Bonferroni adjustments).
...
We used an ANOVA evaluate the effect of region on natural-log transformed scar density (significant) and a Tukey post-hoc comparison for significant differences. The model met the homogeneity and normality assumptions and there were no significant outliers based on the IQR method.
...
```
We used an analysis of variance model to evaluate the effect of region on natural-log transformed percent coral area grazed and Tukey HSD to evaluate regional differences. One site was a significant outlier based on the IQR method, but results were consistent when this site was filtered and analysis was rerun.
...
fish_corals_site_level %>%
  ...
  rstatix::levene_test(lm_mean_pct_coral_grazed ~ region)
...
fish_corals_site_level_filtered <- fish_corals_site_level %>%
    filter(!site %in% c("Teague Bay"))

#No significant (p<0.05) violations of normality assumption after Teague Bay removed
model_pct_grazed_region_filtered <- aov(log(mean_pct_coral_grazed)~region, data = fish_corals_site_level_filtered)
rstatix::shapiro_test(residuals(model_pct_grazed_region_filtered))

#Testing model assumptions: visual assessment of residuals
plot(model_pct_grazed_region) 
plot(model_pct_grazed_region_filtered) #more deviations from QQ-plot when two sites removed for overall model

#Model meets assumptions, so proceeding with interpreting model output
summary(model_pct_grazed_region) #Significant difference in percent coral area grazed
summary(model_pct_grazed_region_filtered) #Model with outlier dropped also significant

#Testing for pairwise differences in scar density by region and estimated marginal means
TukeyHSD(model_scar_density_region) 
TukeyHSD(model_pct_grazed_region_filtered)
```
## Coral taxa selectivty by region

Caluculating genus-based elecitivity across regions using Vanderploeg and Scavia's E * index. This index was chosen based on the recommendations from Lechowicz (1982). We compared both abundance-based elecitity (scar abundance to coral abundance) and area-based elecitivty (scar area to coral area).

Reference: Lechowicz, MJ. (1982) 'The sampling characteristics of electivity indices'. Oecologia. 52:22–30.

In the following figure, raw points show site-level selectivity, while circles with a black outline show the regional mean and lines show the 95% CI.
...
selectivity_summary_stats_genus %>% select(region, coral_genus, abundance_selectivity_sig, area_selectivity_sig)
```

```{r Summarizing grazing intensity by species}
...
#>82% of observed predation was on these four coral species
n_top_4_grazed/n_grazed_cols*100
```

In the following figure, raw points show site-level selectivity of the four most commonly grazed coral taxa, while circles with a black outline show the regional mean and lines show the 95% CI. 80.9% of all observed recent predation scars were found on *Orbicella annularis*, Branching *Porites* spp., *Porites astreoides*, and *Siderastrea siderea*. These were also the only coral taxa with at least 30 observed colonies with predation scars. Given this particularly intensive predation on these four taxa, we evaluated patterns of selectivity across regions for these taxa. 

```{r Supplemental Fig. 1: Coral taxa selectivity by site and region, fig.height = 4.33, fig.width = 8.46}
#raw selectivity data
...
#Variation in corallivory among sites
...
site_level_coral_area_grazed_by_corallivore_density <- fish_corals_site_level %>%
  ...
  scale_x_continuous(expand = c(0.1, 0))+ #values near 0 cutoff, expands x-axis to resolve issue
  labs(x=expression(paste("Major corallivore density (n 100 ", m^-2, ")")), 
       y=expression(paste("Coral area grazed (%)")), 
       color="Region") +
  theme_bw()
...
site_level_coral_area_grazed_by_coral_cover <- fish_corals_site_level %>%
  ...
  scale_y_continuous(trans = scales::log_trans(),
                     breaks= c(0.00025, 0.0025, 0.025, 0.25, 2.5, 0.25),
                     labels= c(0.00025, 0.0025, 0.025, 0.25, 2.5, 0.25))+
  labs(y="Coral area grazed (%)", x="Coral cover (%)", color="Region")+
  theme_bw()+
  theme(legend.position = "top")+
  rremove("y.title") +  
  rremove("y.ticks") + 
...
*Random slope and intercept model:* Initially, we ran models using a random slope and intercept, but the high correlation of the random effects (-1/1) was problematic for model convergence and can compromise the power of the models (Matuschek et al. 2017). This may have been caused by not having enough data to properly estimate the random slope components given the observed variance. Therefore, we used reran the model using a random intercept but not a random slope.
...
*Assessing model assumptions/fit:* Based on a DHARMa nonparametric dispersion test via sd of residuals fitted vs. simulated (n=10,000 simulations), there was no significant overdispersion (p-value=0.71), there was no significant violations of the normality of the residuals (Kolmogorov-Smirnov test, p-value=0.16), and there were no significant outliers (p-value=1). 
...
*Random slope and intercept model:* Initially, we ran models using a random slope and intercept, but the high correlation of the random effects (>0.85) was problematic for model convergence and can compromise the power of the models (Matuschek et al. 2017). Therefore, we used reran the model using a random intercept but not a random slope.
...
proportion_grazed_m1_interaction <- glmmTMB::glmmTMB(log(mean_pct_coral_grazed) ~ mean_density_major_corallivores*mean_pct_coral_cover +
                              (1 | region), #transects nested within sites within regions
                              data = fish_corals_site_level,
                              family=gaussian)

proportion_grazed_m1_interaction %>% summary() #interaction NS, dropping interaction term

#Random-slope model without interaction
...
plot(proportion_grazed_m1_residuals) #sig quantile deviations
...
plot(proportion_grazed_m2_residuals) #no sig deviations and residual vs predicted plot look much better post transformation
...
write_csv(table_s4, here("supplements/table_s4test_site_level_glmm_results.csv"))
```
# ___________________________________________________________________________________
## Transect-level analysis

```{r Data analysis: calculating coral diversity by transect, warning=FALSE, message=FALSE}
Data visualization: Fig 5 (a) Scar area and (b) percent colony grazed by taxa + region, predated_coral_patterns, fig.height = 3.15, fig.width = 8.46}
set.seed(1945) #for reproducible results from random simulations of residuals

total_coral_abundance_and_cover <- coral_scar_df %>% 
  group_by(region, site, transect, depth_m) %>% 
  reframe(total_n_colonies=n(),
          total_pct_coral_cover=(sum(col_area_cm2)/300000)*100)

coral_diversity_and_cover <- coral_scar_df %>% 
  group_by(region, site, transect, depth_m, coral_species) %>% 
  reframe(n_colonies=n(),
          pct_coral_cover=(sum(col_area_cm2)/300000)*100) %>%
  left_join(total_coral_abundance_and_cover) %>%
  mutate(prop_abundance=n_colonies/total_n_colonies,         
         p_ln_prop_abundance=prop_abundance*log(prop_abundance)) %>%
  group_by(region, site, transect, depth_m) %>%
  reframe(coral_diversity=sum(p_ln_prop_abundance)*-1,
          pct_coral_cover=sum(pct_coral_cover))

# Transect-level summary of corallivory for coral species
n_colonies_species_transect <- coral_scar_df %>% 
  group_by(region, site, transect, depth_m, coral_species) %>% 
 dplyr::reframe(n_colonies=n())
  
transect_level_corallivory <- coral_scar_df %>% 
  group_by(region, site, transect, depth_m, coral_species) %>% 
  filter(n_new_scars>0) %>%
  dplyr::reframe(n_colonies_grazed=n()) %>%
  left_join(n_colonies_species_transect) %>%
  filter(coral_species %in% c("Orbicella annularis", "Porites astreoides", "Branching Porites spp.", "Siderastrea siderea"))

#creating a empty data grame with all transects and coral species to later add 0s in
temp_df <- coral_scar_df %>% 
  select(region, site, transect, depth_m) %>% 
  distinct() 

OANN_temp <- temp_df %>%  mutate(coral_species="Orbicella annularis")
PAST_temp <- temp_df %>%  mutate(coral_species="Porites astreoides")
BPOR_temp <- temp_df %>%  mutate(coral_species="Branching Porites spp.")
SSID_temp <- temp_df %>%  mutate(coral_species="Siderastrea siderea")

combined_blank_df <- OANN_temp %>% bind_rows(PAST_temp) %>% bind_rows(BPOR_temp)%>% bind_rows(SSID_temp)

corrected_transect_level_corallivory <- combined_blank_df %>% left_join(transect_level_corallivory) %>%
  mutate(n_colonies_grazed=replace_na(n_colonies_grazed, 0),
         n_colonies=replace_na(n_colonies, 0)) %>%
    left_join(coral_diversity_and_cover)
```
...
n_grazed_abundance_diversity_poisson <- glmmTMB::glmmTMB(n_colonies_grazed ~ coral_species  + abundance_based_diversity+pct_coral_cover+ region+
...
#negative binomial distribution sig better fit for model
anova(n_grazed_abundance_diversity_poisson, n_grazed_abundance_diversity_nbinom) 
...
write_csv(n_scars_col_level_model_output,  here("supplements/table_s5_colony_level_scar_abundance_results.csv"))

n_scars_col_level_model_output %>% filter(p_value<0.05)
```

```{r Data visualization: Fig S2 corals grazed by coral diversity , predated_coral_patterns, fig.height = 3.15, fig.width = 8.46}
...
plot(n_scars_col_level_residuals) #NS overdispersion, but potentially sig. outliers. Package message notes that binomial models may have inflated Type I error rates for interger-values distributions and recommends re-running testOutliers using type = "bootstrap"

#Testing for outliers usin bootstapping (unhash to run, code may take several hours to run)
#n_scars_col_level_outliers <- DHARMa::testOutliers(n_scars_col_level_residuals, type = "bootstrap", nBoot = 1000) #NS outliers
#n_scars_col_level_outliers

#model output
summary_n_scars_col_level_m2 <- n_scars_col_level_nbinom %>%
  summary()
summary_n_scars_col_level_m2 
...
```{r Table S5: Colony-level scar abundance model output}
#tidying model output and calculating confidence intervals
...
predated_colonies_subset <- coral_scar_df %>% 
  ...
  mutate(coral_species=factor(coral_species, levels = c("Agaricia agaricites", "Agaricia humilis", "Agaricia tenuifolia", "Agaricia spp.", "Madracis auretenra", "Madracis decactis", "Orbicella annularis", "Orbicella faveolata", "Branching Porites spp.", "Porites astreoides", "Siderastrea siderea"))) %>%
  mutate(region= factor(region, levels = c("Panamá", "Florida",  "St. Croix", "Bonaire"))) 
```
...
mean_scar_size <- glmmTMB::glmmTMB(log(mean_scar_area_cm2) ~ col_area_cm2 + coral_species + region+
                              (1 | site/transect), #transects nested within sites within regions
                              family=gaussian,
...
mean_scar_size_no_outliers <- glmmTMB::glmmTMB(log(mean_scar_area_cm2) ~ col_area_cm2 + coral_species + region+
                              (1 | site/transect), #transects nested within sites within regions
                              family=gaussian,
...
```{r Table S6: Colony-level mean scar size model output}
...
write_csv(mean_scar_size_col_level_model_output,  here("supplements/table_s6_colony_level_mean_scar_size_results.csv"))

#summarizing mean observed scar size by region
predated_colonies_subset %>% 
  ungroup() %>% 
  group_by(region) %>% 
  dplyr::reframe(mean(mean_scar_area_cm2),
                 sem(mean_scar_area_cm2))
```
...
```{r Table S7: Colony-level percent coral grazed model output}
...
write_csv(pct_grazed_col_level_model_output,  here("supplements/table_s7_colony_level_percent_coral_grazed_results.csv"))
```

```{r Data visualization: Fig 5 (a) Scar area and (b) percent colony grazed by taxa + region, predated_coral_patterns, fig.height = 3.15, fig.width = 8.46}
...
fig_mean_scar_taxa <-predated_colonies_subset %>%
  ...
  mutate(coral_species= factor(coral_species, levels = c("Porites astreoides", "Branching Porites spp.", "Orbicella annularis","Siderastrea siderea","Agaricia agaricites","Agaricia tenuifolia", "Madracis decactis", "Orbicella faveolata", "Agaricia humilis", "Madracis auretenra", "Agaricia spp."))) %>%
  ggplot()+
...
fig_pctgrazed_taxa <- predated_colonies_subset %>%
  ...
  scale_x_continuous(trans = scales::log_trans(),
                     ...
                     limits=c(0.001,75))+
  labs(x = expression(paste("Coral area grazed (%)")),
       y = "",
       color="Region",
       fill="Region")+
  theme_bw()+
