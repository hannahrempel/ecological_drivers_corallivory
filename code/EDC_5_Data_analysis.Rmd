...
```{r}
coral_scar_df %>%
  filter(region=="Panamá" & n_new_scars>0) %>%
  group_by(site) %>%
...
```{r Summary of parrotfish density and biomass by transect; site and region}
...
write.csv(regional_parrotfish_coral_scar_summary, here("figures_tables/table2_regional_patterns.csv"),
          fileEncoding = "UTF-8", 
          row.names=FALSE)
```
...
```
To enable the reader to better understand what is driving these patterns I suggest including plots of the abundance of each coral taxa, and the frequency of scars on each coral genera for each region

```{r Calculating scar density and area grazed by genus for supp figs 3-4}
...
#creating a template dataframe with all unique combinations of region, site, transect, and coral genus (to later account for any 0's in coral observations in a transect)
coral_genera_list <- coral_genus_count_and_area_by_transect %>% 
...
#using unique combos df to add zeros back into dataframe for instances where a coral genera was not observed in a trasect
...
#summary of scar density and area per transect
...
```{r Fig. S3: Density of corals and predation scars, fig.height = 4.33, fig.width = 8.46}
#creating preliminary figure
...
fig_coral_scar_density_combo <- ggpubr::ggarrange(fig_coral_density, 
                                                  fig_scar_density,
                          align = c("v"),
                          labels=c("a", "b"),
...
#saving figure
ggsave(plot=fig_coral_scar_density_final, filename=here("supplements/figs3_coral_scar_density.jpg"), width = 215, height = 110, dpi = 600, units = "mm")
```
**Fig. S3.** Add text...


```{r Fig. S4: Percent cover of corals and predation scars, fig.height = 4.33, fig.width = 8.46}
#creating preliminary figures
...
fig_pct_coral_predated <- ggplot()+
  ...
  scale_fill_manual(values=region_palette)+
  theme_bw()+
  labs(x="Area preyed upon (%)         ", y="Region")+ #spacing after (%) intentional
  facet_grid(~coral_genus)+
  coord_flip()+
  theme(legend.position = "none")
...
#saving figure
ggsave(plot=fig_relative_coral_area_grazed_final, filename=here("supplements/figs4_coral_area_grazed.jpg"), width = 215, height = 110, dpi = 600, units = "mm")
```
