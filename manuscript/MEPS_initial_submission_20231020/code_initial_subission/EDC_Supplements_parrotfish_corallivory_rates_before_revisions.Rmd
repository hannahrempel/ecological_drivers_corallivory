...
fla_grazing_raw <- read_csv(here("data/supp_data/published_fish_foraging_rates_raw/FLA_foraging_data.csv")) %>%
  janitor::clean_names()

stx_grazing_raw <- read_csv(here("data/supp_data/published_fish_foraging_rates_raw/STX_foraging_data.csv")) %>%
  janitor::clean_names()
```
...
fla_coral_predation <- fla_grazing_raw %>% 
  ...
  dplyr::reframe(n_bites_coral=sum(bites))

fla_corallivory_by_fish <- fla_grazing_raw %>% 
  group_by(location, species, fish_id) %>%
  filter(!food_category %in% c("Not_observed", "Unknown")) %>%
  dplyr::reframe(n_bites=sum(bites)) %>% 
...
fla_corallivory_by_species <- fla_corallivory_by_fish %>%
  ...
  dplyr::reframe(mean_corallivory=mean(pct_bites_coral),
            n_observed=n()) %>%
...
write_csv(fla_stx_corallivory_by_species, here("data/supp_data/table_s3_STX_FLA_summarized_corallivory_rates.csv"))
```
...
corallivory_rates_combined <- read_csv(here("data/supp_data/corallivory_by_parrotfish_sp.csv"))
```
...
mean_corallivory_by_pf_sp <- corallivory_rates_combined %>%
  group_by(species) %>%
  uncount(replicates) %>% 
  dplyr::reframe(mean_pct_bites_on_coral=mean(as.numeric(pct_bites_on_coral)),
            replicates=n()) %>%
