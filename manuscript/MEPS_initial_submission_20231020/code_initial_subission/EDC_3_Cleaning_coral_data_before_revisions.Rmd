...
bootstrapped_scar_df <- read.csv(here("data/bootstrapped_new_scar_data.csv"), fileEncoding = "UTF-8")
```


```{r}
bootstrapped_scar_df %>% 
  filter(site=="Salt River") %>% 
  filter(transect==5) %>%
  filter(scar_area_cm_sq==0.002)
  
bootstrapped_scar_dfcorr<-bootstrapped_scar_df %>%
  mutate(scar_area_cm_sq=case_when(colony_id==3476 & scar_area_cm_sq==0.001~0.01,
                                   colony_id==3475 & scar_area_cm_sq==0.002~0.01,
                                   colony_id==3475 & scar_area_cm_sq==0.006~0.06,
                                   TRUE~as.double(scar_area_cm_sq)))


bootstrapped_scar_dfcorr %>% filter(region=="Florida") %>% filter(!is.na(scar_area_cm_sq)) %>% summarise(min(scar_area_cm_sq))
```

```{r Summary of scar observations by coral colony size category}
#subsetting df to only include colonies with new scars
scar_df <- coral_scar_df %>% 
  filter(n_scars>0 & percent_new>0)

#alphabetical list of all grazed coral species (including those with <3 recently predated colonies across the dataset)
scar_df %>% 
...
write_csv(scar_observations_by_sp, here("supplements/table_s2_predation_by_coral_taxa.csv"))
```
...
estimated_col_sizes <- match_by_species %>%
  ...
  #calculating colony are based on SA of a hemisphere
...
#the 6 instances where coral_species had <3 observed recent grazing scars were excluded due to limited data
area_grazed_by_col %>% 
